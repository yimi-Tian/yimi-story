...
export function ContentListPage({ type }: { type: ContentType }) {
  ...
  return <>
    ...
    {failed ? <ErrorState message="無法讀取內容，請確認網路連線與管理員權限。" /> : !items ? <LoadingState label="正在讀取內容" /> : <>
      ...
      <div className="table-scroll"><table className="content-table"><thead><tr>{isClass ? <><th>ID</th><th>標題／課程</th><th>講師</th><th>地區／場地</th><th>年度</th></> : <><th>ID</th><th>活動名稱／日期</th><th>類型／帶領者</th><th>地區／場地</th><th>人數</th></>}<th>狀態</th><th>最後更新</th><th /></tr></thead><tbody>{filtered.map((item) => {

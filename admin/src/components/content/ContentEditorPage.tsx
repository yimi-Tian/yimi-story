...
export function ContentEditorPage({ type, isNew = false }: { type: ContentType; isNew?: boolean }) {
  ...
  return <>
    ...
    <section className="version-panel"><div><span>目前網站版本</span><strong>{record?.publishedAt ? `正式版本${record.publishedRevision ? ` r${record.publishedRevision}` : ""}` : "尚未發布"}</strong>{record?.publishedAt&&<small>版本建立時間 {new Date(record.publishedAt).toLocaleString("zh-TW")}</small>}</div><div><span>最新草稿</span><strong>{record?.revision ? `r${record.revision}` : "建立後為 r1"}</strong><small>{record?.updatedAt ? `最後更新 ${new Date(record.updatedAt).toLocaleString("zh-TW")}` : "尚未儲存"}</small></div><span className={`record-status ${dirty ? "has-draft" : ""}`}>{dirty ? "有尚未儲存的變更" : record?.publishedSnapshotId && record.draftId ? record.publishedData && matchesPublishedContent(record.data, record.publishedData) ? "內容已同步" : "有未發布變更" : status === "validated" ? "可準備發布" : "草稿"}</span></section>

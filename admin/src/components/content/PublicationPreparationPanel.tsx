...
export function PublicationPreparationPanel({ client, contentId, draftId, revision, draftStatus, blocked }: Props) {
  ...
  return <section className="form-section publication-panel" aria-labelledby="publication-title">
    ...
    {preparation && <div className="validation-grid" aria-live="polite">
      <section className="validation-panel validation-panel--error"><h3>阻擋問題（{preparation.errors.length}）</h3>{preparation.errors.length ? <ul>{preparation.errors.map((item, index) => { const issue = presentValidationIssue(item); return <li key={`${item.code}-${index}`}><strong>{issue.field}</strong>：{issue.message}</li>; })}</ul> : <p>最終檢查沒有阻擋問題。</p>}</section>
      <section className="validation-panel validation-panel--warning"><h3>提醒（{preparation.warnings.length}）</h3>{preparation.warnings.length ? <ul>{preparation.warnings.map((item, index) => { const issue = presentValidationIssue(item); return <li key={`${item.code}-${index}`}><strong>{issue.field}</strong>：{issue.message}</li>; })}</ul> : <p>目前沒有提醒。</p>}</section>
    </div>}

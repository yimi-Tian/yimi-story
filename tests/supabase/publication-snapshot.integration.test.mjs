...
function preparationSql(publicId,expectedRevisionExpression="d.revision",checksumOverride=null){return `with d as (select d.* from public.content_drafts d join public.content_items c on c.id=d.content_id where c.public_id=${q(publicId)}), refs as (select d.id draft_id,nullif(d.data->>'coverAssetId','') reference,'cover' role,0::bigint ordinal from d where nullif(d.data->>'coverAssetId','') is not null union all select d.id,value,'gallery',ordinality from d,jsonb_array_elements_text(coalesce(d.data->'galleryAssetIds','[]')) with ordinality), manifest as (select coalesce(jsonb_agg(jsonb_build_object('id',coalesce(m.legacy_asset_key,m.id::text),'mediaId',m.id::text,'role',m.role::text,'sortOrder',r.ordinal,'source',m.source::text,'sha256',m.sha256,'legacyPath',m.legacy_path,'bucket',m.bucket,'objectPath',m.object_path,'originalMediaId',m.original_media_id) order by case r.role when 'cover' then 0 else 1 end,r.ordinal),'[]') value from d join refs r on r.draft_id=d.id join public.media_assets m on m.content_id=d.content_id and m.role::text=r.role and (m.id::text=r.reference or m.legacy_asset_key=r.reference) and m.deleted_at is null) select (public.prepare_publication_snapshot(${q(ACTIVE)},d.id,${expectedRevisionExpression},'1.0',d.data-'internalNotes',manifest.value,jsonb_build_object('valid',true,'revision',d.revision,'errors','[]'::jsonb,'warnings','[]'::jsonb,'mediaCount',jsonb_array_length(manifest.value),'coverReady',true,'exporterDeterministic',true),${checksumOverride??"public.publication_snapshot_checksum('1.0',d.data-'internalNotes',manifest.value,d.revision)"})).id from d,manifest`};
